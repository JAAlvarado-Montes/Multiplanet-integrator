...
###################################################
uM, uL, uT = units(uL=0.005 * AU, uM=MSUN)
...
initial_force_outer = grav_force(m2, m0, SO_separation_vectors[0], SO_separations[0]) + grav_force(m2, m1, IO_separation_vectors[0], IO_separations[0]) + grav_relat_force(m1, m0, c_light, SI_separation_vectors[0], SI_separations[0], inner_velocities[0]) * m2 / m0
...
#######################################
inner_forces = np.zeros((num_steps, 2), dtype=float)
inner_forces[0] = initial_force_inner

###################################
# #### FORCE VECTORS FOR OUTER ####
###################################
outer_forces = np.zeros((num_steps, 2), dtype=float)
outer_forces[0] = initial_force_outer

######################################
# #### FORCE VECTORS FOR THE STAR ####
######################################
star_forces = np.zeros((num_steps, 2), dtype=float)
star_forces[0] = initial_force_star

COM = np.zeros((num_steps, 2), dtype=float)
COM[0] = COM_initial
...
if method_flag == "while":
    ...
    while t <= total_t:
        ...
        outer_forces[1] = grav_force(m2, m0, SO_separation_vectors[0], SO_separations[0]) + grav_force(m2, m1, IO_separation_vectors[0], IO_separations[0]) + grav_relat_force(m1, m0, c_light, SI_separation_vectors[0], SI_separations[0], inner_velocities[0]) * m2 / m0
    ...
    for i in trange(num_steps - 1):
        ...
        # Calculate forces
        inner_forces[i + 1] = grav_force(m1, m0, SI_separation_vectors[i + 1], SI_separations[i + 1]) + grav_force(m1, m2, OI_separation_vectors[i + 1], IO_separations[i + 1]) + grav_relat_force(m1, m0, c_light, SI_separation_vectors[i + 1], SI_separations[i + 1], inner_velocities[i + 1]) * (m0 + m1) / m0
        outer_forces[i + 1] = grav_force(m2, m0, SO_separation_vectors[i + 1], SO_separations[i + 1]) + grav_force(m2, m1, IO_separation_vectors[i + 1], IO_separations[i + 1]) + grav_relat_force(m1, m0, c_light, SI_separation_vectors[i + 1], SI_separations[i + 1], inner_velocities[i + 1]) * m2 / m0
...
print(x_vals_inner[0] * uL / AU, x_vals_inner[-1] * uL / AU)
